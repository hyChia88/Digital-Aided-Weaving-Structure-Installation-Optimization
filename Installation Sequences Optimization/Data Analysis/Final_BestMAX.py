...
# Calculate range_value for each sequence
max_values = {sequence: max(results) for sequence, results in results_data.items()}

#set m, n
n=25
m=3
# Sort sequences based on their RANGE_value and get the top n sequences
sorted_sequences = sorted(max_values.items(), key=lambda x: x[1])[:n]
...
# Extract the first half of tags# from the top n sequences
...
for sequence, _ in sorted_sequences:
    ...
    # Calculate the weight for RANGE_value (lower is better)
    max_values_weight = 1 / max_values[sequence]
    ...
    for tag in sequence_tags:
        influence_factors[tag] += max_values_weight * sequence_weight[tag]
...
for tag, total_influence_factor in sorted_influence_factors:
    print("{}, Total Influence Factor (MAX): {}".format(tag, total_influence_factor))
...
for tag, total_influence_factor in sorted_influence_factors:
    print("{}".format( total_influence_factor))