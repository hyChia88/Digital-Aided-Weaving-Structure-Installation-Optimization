...
range_values = {sequence: max(results) - min(results) for sequence, results in results_data.items()}

#set m, n
n=25
m=3
# Sort sequences based on their RANGE_value and get the top n sequences
sorted_sequences = sorted(range_values.items(), key=lambda x: x[1])[:n]
...
for sequence, _ in sorted_sequences:
    ...
    range_values_weight = 1 / range_values[sequence]
    ...
    for tag in sequence_tags:
        influence_factors[tag] += range_values_weight * sequence_weight[tag]
